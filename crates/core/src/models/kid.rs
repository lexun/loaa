...
impl Kid {
    pub fn new(name: String, account_id: Uuid, owner_id: String) -> Result<Self> {
        let kid = Self {
            id: Uuid::new_v4(),
...
mod tests {
    ...
    fn test_kid_creation() {
        ...
        let kid = Kid::new("Alice".to_string(), account_id, "test-owner".to_string()).unwrap();
        assert_eq!(kid.name, "Alice");
        assert_eq!(kid.account_id, account_id);
    ...
    fn test_kid_validation_empty_name() {
        let result = Kid::new("   ".to_string(), Uuid::new_v4(), "test-owner".to_string());
        assert!(result.is_err());
    }

    #[test]
    fn test_kid_validation_trimmed_name() {
        let kid = Kid::new("  Bob  ".to_string(), Uuid::new_v4(), "test-owner".to_string()).unwrap();
