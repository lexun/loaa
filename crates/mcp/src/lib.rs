//! Loaa MCP Server Library
//!
//! This crate provides the MCP (Model Context Protocol) server implementation for Loaa.
//! It can be used as a standalone server or embedded into the web server.
...
/// Run the MCP server with stdio transport (for local CLI use)
pub async fn run_stdio_server(server: LoaaServer) -> Result<()> {
...
/// Create the MCP HTTP router for embedding in another server
/// This returns a Router that can be nested into an existing Axum app
pub fn create_mcp_router(server: LoaaServer) -> axum::Router {
    use axum::{Router, middleware};
    use rmcp::transport::streamable_http_server::{
        StreamableHttpService,
        session::local::LocalSessionManager,
    ...
    // Create router with MCP endpoint and JWT validation middleware
    Router::new()
        .nest_service("/mcp", service)
        .layer(middleware::from_fn(auth::validate_jwt))
}
